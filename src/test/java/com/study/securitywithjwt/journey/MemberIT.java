...
public class MemberIT {
  ...
  void roleUserCrud() {
    ...
    //update test member - update own data with role user - return new tokens - 200
    ...
    LoginResponseDto loginResponseDtoOfResponse = webTestClient.put()
    ...
    assertThat(loginResponseDtoOfResponse).isNotNull();
    assertThat(loginResponseDtoOfResponse.getName()).isEqualTo(changedName).isEqualTo(savedRoleUserAfterUpdate.getName());
    assertThat(loginResponseDtoOfResponse.getEmail()).isEqualTo(changedEmail).isEqualTo(savedRoleUserAfterUpdate.getEmail());

    String accessToken = loginResponseDtoOfResponse.getAccessToken();

    Claims claimsFromAccessToken = jwtUtils.extractClaimsFromAccessToken(accessToken);
    assertThat(claimsFromAccessToken.getSubject()).isEqualTo(updateRequestDto.getEmail());
    assertThat(claimsFromAccessToken.get("name")).isEqualTo(updateRequestDto.getName());
  ...
  void roleAdminCrud() {
    ...
    roleAdminSignupRequest.setRoles(userRoleSet);
