...
public class MemberController {
  ...
  public ResponseEntity<Page<MemberDto>> getAllMembers(@RequestParam(name = "page", defaultValue = "1") Integer page,
    ...
    if (memberDtoPage.getNumberOfElements() == 0) {
      throw new ResourceNotFoundException("No Contents found for requested page %d and size %d.".formatted(page, size));
    }
  ...
  public ResponseEntity<?> updateMember(@Valid @RequestBody MemberUpdateRequestDto updateRequestDto, //최상단으로
    ...
    log.info("Update request successful. Target member(memberId: {}) info updated by member(memberId: {}, roles: {})",
        memberId, loginMember.getMemberId(), loginMember.getRoles());
    Member updatedMember = memberService.updateMember(memberId, updateRequestDto);

    //사용자 본인의 정보를 수정한 경우, 재 로그인처리로 새로운 토큰을 포함한 응답
    if (memberId.equals(loginMember.getMemberId())) {
      LoginRequestDto updatedMemberLoginRequest = new LoginRequestDto(updatedMember.getEmail(), updateRequestDto.getPassword());
